...
def cmd_standalone(project_folder, platform, api, test_platform_args):
    ...
    base.extend([
        f'cd {TEST_PROJECTS_DIR}/{project_folder} && DISPLAY=:0.0 ./utr {test_platform_args}Linux64 --artifacts_path={PATH_TEST_RESULTS} --timeout=1200 --player-load-path=./{PATH_PLAYERS} --player-connection-ip=auto{_get_extra_utr_arg(project_folder)}'
      ])
    return base
